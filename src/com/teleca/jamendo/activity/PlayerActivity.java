...
/**
 * Central part of the UI. Touching cover fades in 4-way media buttons. 4-way media buttons fade out after certain
 * amount of time. Other parts of layout are progress bar, total play time, played time, song title, artist name and
 * jamendo slider.<br>
 * <br>
 * 
 * License information is implemented overlaying CreativeCommons logo over the album picture. Information about type of
 * license is retrieved concurrently to track bufferring.
 * 
...
public class PlayerActivity extends Activity {
    ...
    public static void launch(Context c, Playlist playlist) {
        ...
        /*
         * For example, consider a task consisting of the activities: A, B, C, D. If D calls startActivity() with an
         * Intent that resolves to the component of activity B, then C and D will be finished and B receive the given
    ...
    public static void launch(Activity c, PlaylistRemote playlistRemote) {
        Intent intent = new Intent(c, PlayerActivity.class);
        new PlaylistRemoteLoadingDialog(c, R.string.loading_playlist, R.string.loading_playlist_fail, intent)
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mGesturesOverlayView.addOnGesturePerformedListener(JamendoApplication.getInstance().getPlayerGestureHandler());
    }
     ...
     * next button action
     */
    private OnSeekToListenerImp mOnForwardTouchListener = new OnSeekToListenerImp(this, getPlayerEngine(),
            SeekToMode.EForward);

    /**
     * prev button action
     */
    private OnSeekToListenerImp mOnRewindTouchListener = new OnSeekToListenerImp(this, getPlayerEngine(),
            SeekToMode.ERewind);
    ...
    private void handleIntent() {
        ...
        if (getIntent().getData() != null) {
            ...
            if (!getIntent().getBooleanExtra("handled", false)) {
                mUriLoadingDialog = (LoadingDialog) new UriLoadingDialog(this, R.string.loading, R.string.loading_fail)
                        .execute();
            }
    ...
    /**
     * This creates playlist based on url that was passed in the intent, e.g. http://www.jamendo.com/pl/track/325654 or
        ...
        public Playlist doInBackground(Void... params) {
            ...
            if (Intent.ACTION_VIEW.equals(action)) {
                ...
                if (mode.equals("track")) {
                    try {
                        Track[] tracks = service.getTracksByTracksId(new int[] { id }, JamendoApplication.getInstance()
                                .getStreamEncoding());
                ...
                if (mode.equals("album")) {
                    try {
                        ...
                        Track[] tracks = service.getAlbumTracks(album, JamendoApplication.getInstance()
                                .getStreamEncoding());
    ...
    public void licenseClickHandler(View v) {
        if (mLicense != null) {
            Intent myIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(mLicense.getUrl()));
